...
private var logger = Logger(subsystem: "co.hugginface.diffusers", category: "Downloader")
class Downloader: NSObject, ObservableObject {
    ...
    private(set) lazy var downloadState:
        CurrentValueSubject<DownloadState, Never> = CurrentValueSubject(
            .notStarted
        )
    ...
    init(from url: URL, to destination: URL, using authToken: String? = nil) {
        ...
        var config = URLSessionConfiguration.default
            ...
            config = URLSessionConfiguration.background(
                withIdentifier: "net.pcuenca.diffusion.download"
            )
        ...
        urlSession?.getAllTasks { tasks in
            ...
            guard tasks.filter({ $0.originalRequest?.url == url }).isEmpty
            else {
...
extension Downloader: URLSessionDelegate, URLSessionDownloadDelegate {
    ...
    func urlSession(
        ...
        guard FileManager.default.fileExists(atPath: location.path) else {
            downloadState.value = .failed(
                "Invalid download location received: \(location)"
            )
            return
        }
        do {
    ...
    func urlSession(
        ...
        if let error = error {
            ...
            logger.warning("HTTP response status code: \(response.statusCode)")
            if response.statusCode >= 300 {
                logger.warning("Session task failed: \(response.statusCode)")
                downloadState.value = .failed(
